...
class GroupchatService(APIService):
    ...
    def register_update_chat(self, chat: Chat) -> str:
        ...
        raise Exception("Failed to register groupchat.")
    ...
    def unregister_chat(self, chat: Chat) -> str:
        ...
        raise Exception("Failed to unregister groupchat.")

    def get_chats(self) -> list:
        """
        Returns a list of registered groupchats for SSA Announcements.
        """
        response = requests.get(self.base_url, headers=self.headers)
        if response.status_code == 200:
            return response.json()
        raise Exception("Failed to get groupchats.")
